...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('landscapes.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('landscapes.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('landscapes.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
